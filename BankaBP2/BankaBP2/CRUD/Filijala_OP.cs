...
{
  public  class Filijala_OP
    {

        //CRUD
        public void AddFil(string ime,int id_banke)
        {
            if (ime != null)
            {
                using (var db = new Model1Container())
                {
                    Filijala filijal = new Filijala();
                    filijal.ADR_FIL = ime;
                    filijal.BankaID_Banka = id_banke;

                    db.Filijalas.Add(filijal);
                    db.SaveChanges();
                }
            }
        ...
        //Read
        public IEnumerable<Filijala> ReadFil()
        {
            try
            {
                IEnumerable<Filijala> filijalas;

                using (var db = new Model1Container())
                {
                    filijalas = db.Filijalas.Select(x => x);

                    if (filijalas.Any())
                    {
                        filijalas = filijalas.Select(x => x).ToList();
                    }
        ...
        //Update
        public void UpdateBank(int FIL_ID,string ADR_FIL,int Banka_ID)
        {

            using (var db = new Model1Container())
            {
                var banka = db.Filijalas.SingleOrDefault(n => n.ID_FIL == FIL_ID);

                if (banka != null)
                {
                    banka.ID_FIL = FIL_ID;
                    banka.ADR_FIL=ADR_FIL;
        ...
        //Delete
        public void DeleteFilijala(Filijala f)
        {
